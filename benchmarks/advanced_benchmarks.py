...
import sys
sys.path.append('.')
from src.autogen_voting import VotingGroupChat, VotingMethod
...
class AdvancedBenchmarkSuite:
    ...
    async def create_voting_agents(self, count: int, responses: List[str]) -> List[ChatAgent]:
        ...
        return [
            AssistantAgent(f"Agent_{i}", model_client=model_client)
    ...
    async def benchmark_voting_orchestration(
        ...
        # Create agents
        responses = [
            f"I propose we implement {scenario}",
    ...
    async def benchmark_competitor_framework(
        self, 
        framework: str, 
        scenario: str, 
        agent_count: int
    ...
    async def run_adversarial_tests(self) -> List[BenchmarkResult]:
        ...
        for scenario_name, scenario_desc in self.adversarial_scenarios.items():
            ...
            if scenario_name == "byzantine_agents":
                ...
                result = await self.benchmark_voting_orchestration(
                    scenario_desc, 10, VotingMethod.QUALIFIED_MAJORITY
                )
                ...
                result = await self.benchmark_voting_orchestration(
                    scenario_desc, 6, VotingMethod.UNANIMOUS
                )
    ...
    async def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        voting_methods = [VotingMethod.MAJORITY, VotingMethod.QUALIFIED_MAJORITY, 
                         VotingMethod.UNANIMOUS, VotingMethod.PLURALITY]
        
        for method in voting_methods:
            result = await self.benchmark_voting_orchestration(
                "technical_evaluation", 7, method
            )
    ...
    async def generate_visualization_report(self, results_file: Path) -> None:
        ...
        sns.barplot(data=df.groupby("framework")["consensus_quality"].mean().reset_index(), 
                   x="framework", y="consensus_quality", ax=axes[0, 1])
        ...
        axes[2, 0].plot(scalability["agent_counts"], scalability["avg_execution_times"], 
                       marker='o', linewidth=2, label="AutoGen Voting")
