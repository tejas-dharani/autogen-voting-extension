...
class AuditEvent:
    ...
    # Event-specific data
    event_data: Dict[str, Any] = field(default_factory=lambda: {})
...
class AuditLogger:
    ...
    def log_security_violation(self, violation_type: str, details: str) -> None:
        ...
        event = AuditEvent(
            ...
            event_id=SecurityValidator.generate_secure_nonce(),
            event_data={"violation_type": violation_type, "details": details},
        )
        self.log_event(event)

    def log_proposal_created(self, proposal_id: str, agent_name: str, title: str) -> None:
        """Log a proposal creation event."""
        event = AuditEvent(
            event_type=AuditEventType.PROPOSAL_CREATED,
            timestamp=datetime.now(),
            event_id=SecurityValidator.generate_secure_nonce(),
            event_data={"proposal_id": proposal_id, "agent_name": agent_name, "title": title},
        )
        self.log_event(event)
...
class ComplianceReporter:
    ...
    def _verify_data_integrity(self, events: List[AuditEvent]) -> bool:
        ...
        for event in events:
            expected_checksum = SecurityValidator.hash_sensitive_data(
                f"{event.event_type.value}:{event.timestamp.isoformat()}:{event.agent_name}"
            )[:16]
