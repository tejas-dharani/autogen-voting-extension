    ...
    def __init__(self):
        self.classification_rules = {
            ...
            'uncertain_vote': {
                ...
                'patterns': ['abstain', 'clarification']
            }
        }
    ...
    def __init__(self):
        self.insight_patterns = {
            'expertise_indicators': [
                'experience', 'background', 'expertise', 'knowledge',
                'familiar', 'worked with', 'seen this before'
            ],
            'time_constraints': [
                'deadline', 'urgent', 'quickly', 'asap', 'time',
                'schedule', 'timeline', 'rush'
            ],
            'collaboration_signals': [
                'team', 'together', 'collaborate', 'discuss',
                'work with', 'coordinate', 'align'
            ]
        }
...
class VoteUnderstandingEngine:
    ...
    def understand_batch_votes(
        ...
        for voter_name, message_text in vote_messages:
            try:
                results[voter_name] = self.understand_vote_message(
                    voter_name, message_text, context
                )
    ...
    def get_parsing_statistics(self) -> ParsingStatistics:
        ...
        semantic_successes = sum(
            ...
            if not result['semantic_interpretation'].fallback_used
        )
        
        avg_confidence = sum(
            result['semantic_interpretation'].confidence 
            for result in self.processing_history
        ) / total_processed
        
        avg_processing_time = sum(
            result['semantic_interpretation'].processing_time_ms 
            for result in self.processing_history
        ) / total_processed
        
        # Build distributions
        confidence_dist = defaultdict(int)
        intention_dist = defaultdict(int)
        ...
        contextual_elements = {
            'conditions': sum(
                len(result['semantic_interpretation'].conditions_identified)
                for result in self.processing_history
            ),
            'concerns': sum(
                len(result['semantic_interpretation'].concerns_expressed)
                for result in self.processing_history
            ),
            'questions': sum(
                len(result['semantic_interpretation'].questions_raised)
                for result in self.processing_history
            )
        }
    ...
    def _create_error_result(self, voter_name: str, error_message: str) -> Dict[str, Any]:
        ...
        from .semantic_interpreter import SemanticVoteResult, VoteIntention, ConfidenceLevel
