...
class DeliberationMessage(TypedDict):
    """Type definition for deliberation messages."""
...
class DeliberationRound:
    ...
    messages: List[DeliberationMessage] = field(default_factory=lambda: [])
    key_insights: List[str] = field(default_factory=lambda: [])
    concerns_identified: List[str] = field(default_factory=lambda: [])
    alternatives_proposed: List[str] = field(default_factory=lambda: [])
    convergence_metrics: Dict[str, float] = field(default_factory=lambda: {})
    ...
    def add_message(self, speaker: str, content: str, timestamp: Optional[float] = None) -> None:
        ...
        message_entry: DeliberationMessage = {
            "speaker": speaker,
            "content": content,
...
class ConvergenceAnalyzer:
    ...
    def update_participant_position(self, speaker: str, message: str) -> None:
        ...
        if total_indicators > 0:
            ...
            current_position["support"] = alpha * support_ratio + (1 - alpha) * current_position.get("support", 0.5)
            current_position["opposition"] = alpha * opposition_ratio + (1 - alpha) * current_position.get(
                "opposition", 0.5
            )
            current_position["neutral"] = alpha * neutral_ratio + (1 - alpha) * current_position.get("neutral", 0.5)
    ...
    def calculate_current_convergence(self) -> float:
        ...
        convergence_score = 1.0 - min(1.0, float(combined_variance * 4))  # Scale factor of 4
...
class StructuredDeliberationEngine:
    ...
    def start_new_round(self, round_number: int, participants: List[str]) -> DeliberationRound:
        ...
        deliberation_round = DeliberationRound(
            round_number=round_number, participants=participants, start_timestamp=time.time()
        )
    ...
    def process_deliberation_message(self, round_obj: DeliberationRound, speaker: str, message: str) -> None:
    ...
    def complete_current_round(self) -> Optional[DeliberationRound]:
        ...
        if current_round.start_timestamp:
            current_round.duration_seconds = current_round.end_timestamp - current_round.start_timestamp
    ...
    def generate_comprehensive_summary(self) -> DeliberationSummary:
        ...
        all_messages: List[DeliberationMessage] = []
    ...
    def _calculate_overall_participation_balance(self, all_messages: List[DeliberationMessage]) -> float:
    ...
    def _calculate_deliberation_quality(self) -> float:
        ...
        quality_score = convergence_score * 0.4 + participation_score * 0.3 + content_quality * 0.3
...
class ContentInsightExtractor:
    ...
    def _extract_content_snippet(self, message: str, indicators: set[str]) -> str:
...
class MessageInsights:
    ...
    key_points: List[str] = field(default_factory=lambda: [])
    concerns: List[str] = field(default_factory=lambda: [])
