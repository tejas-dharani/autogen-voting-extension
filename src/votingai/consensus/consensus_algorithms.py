...
from .adaptive_strategies import (
    ComplexityClassifier, AdaptiveStrategySelector, 
    DecisionComplexity, ConsensusStrategy, ContextualMetrics
)
from .deliberation_engine import StructuredDeliberationEngine, DeliberationSummary
...
class LearningFramework:
    ...
    def __init__(self, max_history_size: int = 100):
        ...
        self.strategy_success_rates: Dict[str, List[float]] = defaultdict(list)
    ...
    def _calculate_efficiency_ratio(
        ...
        return predicted_duration / max(actual_duration, 0.1)  # Avoid division by zero
    ...
    def _calculate_success_score(self, performance_record: Dict[str, Any]) -> float:
        ...
        return (success * 0.4 + efficiency * 0.2 + satisfaction * 0.3 + convergence * 0.1)
        ...
        for strategy, success_scores in self.strategy_success_rates.items():
            if success_scores:
                insights[strategy] = {
                    ...
                    'consistency': 1.0 - (max(success_scores) - min(success_scores)) if len(success_scores) > 1 else 1.0,
    ...
    def suggest_strategy_adjustments(self) -> List[str]:
        ...
        for strategy, metrics in performance_insights.items():
            ...
            if metrics['consistency'] < 0.7:
                suggestions.append(
                    f"Strategy {strategy} shows inconsistent performance - review context factors"
                )
...
class SmartConsensusOrchestrator:
    ...
    async def analyze_and_recommend(
        ...
        strategy, configuration = self.strategy_selector.select_optimal_strategy(
            complexity, 
            self._prepare_strategy_context(contextual_metrics, participants)
        )
    ...
    def _enrich_metrics_with_context(
        self, 
    ...
    def get_performance_insights(self) -> PerformanceInsights:
        ...
        if strategy_performance:
            best_strategy = max(strategy_performance.keys(), 
                              key=lambda s: strategy_performance[s]['average_success_score'])
