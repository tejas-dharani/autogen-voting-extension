...
class QualityMetrics:
    ...
    def calculate_overall_quality(self) -> float:
        ...
        decision_score = (self.decision_accuracy + self.consensus_strength + self.participant_satisfaction) / 3

        process_score = (self.deliberation_depth + self.argument_quality + self.information_utilization) / 3

        outcome_score = (self.solution_completeness + self.implementation_feasibility + self.stakeholder_alignment) / 3
...
class BenchmarkMetrics:
    ...
    # Additional context
    experimental_conditions: Dict[str, Any] = field(default_factory=lambda: {})
    custom_metrics: Dict[str, float] = field(default_factory=lambda: {})
    ...
    def get_summary_scores(self) -> Dict[str, float]:
        ...
        return {
            ...
            "communication_efficiency": 1.0 / max(1, self.performance.total_messages / max(1, self.participant_count)),
...
class MetricsCollector:
    ...
    def start_collection(self, scenario_name: str, voting_method: str, participant_count: int) -> None:
        """Initialize metrics collection for a benchmark run."""
        self.current_metrics = BenchmarkMetrics(
            scenario_name=scenario_name, voting_method=voting_method, participant_count=participant_count
        )
    ...
    def record_message(self, sender: str, content: str, message_type: str = "standard") -> None:
    ...
    def record_api_call(self, tokens_used: int, estimated_cost: float = 0.0) -> None:
    ...
    def record_decision_outcome(self, decision_reached: bool, final_result: Any = None) -> None:
        ...
        if self.message_log:
            ...
            self.current_metrics.performance.average_message_length = (
                total_length / len(self.message_log) if self.message_log else 0
            )

            self.current_metrics.performance.messages_per_participant = len(self.message_log) / max(
                1, self.current_metrics.participant_count
            )
    ...
    def add_quality_assessment(self, quality_metrics: QualityMetrics) -> None:
    ...
    def add_consensus_assessment(self, consensus_metrics: ConsensusMetrics) -> None:
...
class ComparisonResults:
    ...
    # Comparison analysis
    quality_comparison: Dict[str, float] = field(default_factory=lambda: {})
    performance_comparison: Dict[str, float] = field(default_factory=lambda: {})
    consensus_comparison: Dict[str, float] = field(default_factory=lambda: {})
    overall_winner: str = ""

    # Statistical significance
    significance_tests: Dict[str, Dict[str, float]] = field(default_factory=lambda: {})
    confidence_intervals: Dict[str, Tuple[float, float]] = field(default_factory=lambda: {})
    ...
    def _calculate_comparisons(self) -> None:
        ...
        self.quality_comparison = {
            "overall_quality_ratio": a_quality / max(0.001, b_quality) if b_quality > 0 else 1.0,
            "consensus_strength_ratio": (
                self.system_a_metrics.consensus.final_agreement_level
                / max(0.001, self.system_b_metrics.consensus.final_agreement_level)
            )
            if self.system_b_metrics.consensus.final_agreement_level > 0
            else 1.0,
            "satisfaction_ratio": (
                self.system_a_metrics.quality.participant_satisfaction
                / max(0.001, self.system_b_metrics.quality.participant_satisfaction)
            )
            if self.system_b_metrics.quality.participant_satisfaction > 0
            else 1.0,
        }
        ...
        self.performance_comparison = {
            "time_ratio": (
                self.system_a_metrics.performance.total_duration_seconds
                / max(0.1, self.system_b_metrics.performance.total_duration_seconds)
            ),
            "message_ratio": (
                self.system_a_metrics.performance.total_messages
                / max(1, self.system_b_metrics.performance.total_messages)
            ),
            "token_ratio": (
                self.system_a_metrics.performance.total_tokens / max(1, self.system_b_metrics.performance.total_tokens)
            ),
            "cost_ratio": (
                self.system_a_metrics.performance.estimated_cost_usd
                / max(0.001, self.system_b_metrics.performance.estimated_cost_usd)
            )
            if self.system_b_metrics.performance.estimated_cost_usd > 0
            else 1.0,
        }
        ...
        self.consensus_comparison = {
            "convergence_ratio": (
                self.system_a_metrics.consensus.convergence_rate
                / max(0.001, self.system_b_metrics.consensus.convergence_rate)
            )
            if self.system_b_metrics.consensus.convergence_rate > 0
            else 1.0,
            "participation_balance_ratio": (
                self.system_a_metrics.consensus.participation_balance
                / max(0.001, self.system_b_metrics.consensus.participation_balance)
            )
            if self.system_b_metrics.consensus.participation_balance > 0
            else 1.0,
            "rounds_ratio": (
                self.system_b_metrics.consensus.rounds_to_consensus  # Inverted: fewer rounds is better
                / max(1, self.system_a_metrics.consensus.rounds_to_consensus)
            )
            if self.system_a_metrics.consensus.rounds_to_consensus > 0
            else 1.0,
        }
    ...
    def _determine_winner(self) -> None:
        ...
        weights = {"quality": 0.4, "performance": 0.3, "consensus": 0.3}

        a_scores = self.system_a_metrics.get_summary_scores()
        b_scores = self.system_b_metrics.get_summary_scores()

        a_weighted = sum(a_scores[key] * weights.get(key.split("_")[0], 0.1) for key in a_scores.keys())
        b_weighted = sum(b_scores[key] * weights.get(key.split("_")[0], 0.1) for key in b_scores.keys())
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality_advantage": self.system_a_name
            if self.quality_comparison["overall_quality_ratio"] > 1.1
            else self.system_b_name
            if self.quality_comparison["overall_quality_ratio"] < 0.9
            else "tie",
            "efficiency_advantage": self.system_a_name
            if self.performance_comparison["time_ratio"] < 0.9
            else self.system_b_name
            if self.performance_comparison["time_ratio"] > 1.1
            else "tie",
            "consensus_advantage": self.system_a_name
            if self.consensus_comparison["convergence_ratio"] > 1.1
            else self.system_b_name
            if self.consensus_comparison["convergence_ratio"] < 0.9
            else "tie",
            "key_metrics": {
